...
- 系统音频录制，不以浏览器扩展作为主路径。
- 英文 -> 简体中文优先，预留多语种。
- MVP 主线先做实时字幕与字幕纠错；当前已按负责人确认增加第一版中文同传实验能力。
- 音频、原文、译文、修订历史和同传译音音轨落盘，默认保留 30 天。
- Alpha 阶段使用邀请码 + 用量统计，不先接正式支付。
- 模型与云服务优先使用阿里云。
- **MVP Alpha 快速实现优先**：暂不做 Redis、限流、额度拦截、后台运营；先打通采集 → 字幕 → 落盘 → 导出（详见 [实现说明](./notes-mvp-implementation.md)）。
